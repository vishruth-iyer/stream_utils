...
pub(crate) fn impl_consumer_group(consumer_group_struct: &syn::ItemStruct, output_struct_ident: &syn::Ident, error_ty: &syn::Type) -> syn::ItemImpl {
    let consumer_group_ident = &consumer_group_struct.ident;
    let (impl_generics, ty_generics, where_clause) = consumer_group_struct.generics.split_for_impl();
    ...
    for member in consumer_group_struct.fields.members() {
        let (consumer_member, consumer_future_ident, consumer_result_ident, output_struct_init_line) = match member {
            syn::Member::Named(consumer_ident) => {
                ...
                let output_struct_init_line = quote::quote! { #consumer_ident : #consumer_result_ident ? };
                (syn::Member::Named(consumer_ident), consumer_future_ident, consumer_result_ident, output_struct_init_line)
            }
            syn::Member::Unnamed(consumer_index) => {
                ...
                (syn::Member::Unnamed(consumer_index), consumer_future_ident, consumer_result_ident, output_struct_init_line)
            }
        };
